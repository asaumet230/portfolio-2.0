...
export const CompressActions = ({ filePreviews, handleCompressFiles, handleAddMoreFiles, handleReset, isLoading }: CompressActionsProps) => {
    ...
    return (
        <div className='flex flex-col gap-4 justify-center mx-auto w-10/12 my-10 sm:flex-row sm:w-7/12 md:w-9/12 animate__animated animate__fadeIn'>
            ...
            <button
                disabled={isLoading || filePreviews.length >= 20}
                className={`flex justify-center items-center gap-2 cursor-pointer w-full p-3 font-semibold rounded-md text-base transition-colors
                    ${(isLoading || filePreviews.length >= 20) ? 'bg-indigo-800 text-gray-200 cursor-not-allowed' : 'bg-indigo-600 text-white hover:bg-indigo-700'}`}
