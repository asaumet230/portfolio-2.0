...
export default function ProjectosMovilesPage() {
  ...
  const fetchProjects = async () => {
    try {
      setLoading(true);
      const response = await apiClient.get('/projects/category/mobil');
  ...
  const handleEdit = (project: Project) => {
    router.push(`/dashboard/proyectos/projectos-moviles/${project.slug}`);
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      clearApiCache('GET', '/projects/category/mobil');
      fetchProjects();
    } catch (error: any) {
