...
export default function EditArticuloPage() {
  ...
  const [initialSeo, setInitialSeo]               = useState<SeoForm>({
  ...
  const fetchData = async () => {
    try {
      ...
      const [articleRes, catRes] = await Promise.all([
        apiClient.get(`/articles/${slug}`, token),
        apiClient.get('/article-categories'),
      ]);
      setCategories(catRes.categories || []);
      const a = articleRes.article;
      if (!a) { toast.error('Artículo no encontrado'); router.push('/dashboard/articulos'); return; }
      ...
      if (s.ogImage) {
        const parts = (s.ogImage as string).split('/');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const token = (session as any)?.accessToken;
      await Promise.all([
        apiClient.put(`/articles/${articleId}`, {
        ...
        apiClient.put(`/articles/${articleId}/seo`, {
          ...seo,
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100 truncate">
            {form.title || 'Editar artículo'}
          </h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">/articulo/{form.slug || '...'}</p>
        </div>
        <button
